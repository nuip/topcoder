...
public:
  double expectedTime(int N, int M, int goalX, int goalY) {
    int i,j,k,n,m,r,l,c=105;
    for(i=0;i<N*M;i++){
      if( (goalX-i)%N==0 && (goalY-i)%M==0)
        break;
    }
    if(i==N*M)
      return -1;
    ...
    double re=0,dp[2][212];
    for(i=0;i<2;i++)
      for(j=0;j<212;j++)dp[i][j]=0;
    dp[0][c]=1;
    for(i=1;i<1000000;i++){
      for(j=c-l;j<=c+r;j++){
        dp[i%2][j]=(dp[(i-1)%2][j-1]+dp[(i-1)%2][j+1])/2;
      }
      if(dp[i%2][c-l]!=0){
        re+=i*dp[i%2][c-l];
        dp[i%2][c-l]=0;
